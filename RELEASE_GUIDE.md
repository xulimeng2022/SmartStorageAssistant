...
## 七、T-011 / T-013 候选包补充检查

- [ ] 从正式 v1.1.0 覆盖安装候选包，确认物品、回收站、多图和文字颜色保留。
- [ ] 图片索引单张创建、删除、失败重试；PENDING/PROCESSING 状态不得因无提示点击而删除，OUTDATED 应刷新而不是删除，退出重进状态一致。
- [ ] 批量识别后合并为一条并选择照片，取消合并不改变原草稿。
- [ ] 验证“充电线，放在书桌，第二个抽屉”、“一个充电器，两根数据线”和“充电器放在柜子里，抽屉里的钥匙”的拆分边界。
- [ ] 检查更新的“最新 / 有更新 / 失败”三种状态；每一步只出现一个 Dialog。
- [ ] 完全删除只使用隔离测试数据，两步确认各只出现一个 Dialog，删除后重启回到首次引导并可重新使用。
- [ ] 关闭 AI 图片理解后，Top-5 复核不得继续为剩余候选发送新的视觉请求。
- [ ] 校园邮箱在有/无邮件应用时分别能打开或复制；复制提示与当前语言一致。
- [ ] 简中、繁中、English 和深色模式检查新增界面。
