...
- JVM 单元测试：99 个通过（包含视觉探测、索引状态、语音纠错、合并、导入清理、版本判断和本地化回归用例）。
- Debug 构建：通过。
...
- **v1.2.0（versionCode 6）**：新增照片视觉找物与可选本地图片索引，可按需为单张照片建立或删除索引；批量录入支持把选中的多个物品合并为一条并选择合并照片；语音输入会自动处理多余逗号、停顿和重复词；新增校园邮箱、GitHub 检查更新和“完全删除 App 数据”；修复三语言提示、视觉模型探测误判，并保护 v1.1.0 升级数据。
