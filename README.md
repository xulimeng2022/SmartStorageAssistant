...
- 当前候选包：`app/release/智能收纳助手-1.2.0-rc2.apk`，SHA-256 `11A2F25E08F2950DD48CEBBFB02B8B30B4DC16766FADA8F3372B1EBB7DCF2BF7`；与当前测试包同包名、同签名，v2+v3 校验通过。
- Debug 构建：通过。
...
- **v1.2.0（versionCode 6）**：新增照片视觉找物与可选本地图片索引，可按需为单张照片建立或删除索引；批量录入支持把选中的多个物品合并为一条并选择合并照片，合并预览会包含本次会话尚未分配的照片；语音输入会自动处理多余逗号、停顿和重复词；新增校园邮箱联系入口与标签、GitHub 检查更新和“完全删除 App 数据”；修复设置项长说明挤压标题、三语言提示、视觉模型探测误判，并保护 v1.1.0 升级数据。
